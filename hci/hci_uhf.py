...
For an efficient heat-bath CI program, use the Dice program (https://github.com/sanshar/Dice.git).''')
    ...
    def enlarge_space(cls, hcivecs, add_thresh, config_info, excitation_entries, h1e, eri_mo):
        ...
        for exc_vec in hcivecs[1:]:
            add_list_doubles_exc = hcilib.enlarge_space_doubles(exc_vec, add_thresh, config_info, excitation_entries)
            add_list_singles_exc = hcilib.enlarge_space_singles(exc_vec, add_thresh, config_info, h1e, eri_mo)
            add_list = np.unique(np.concatenate([add_list, add_list_doubles_exc, add_list_singles_exc]))
    ...
    def kernel(self, hcore, eri_ao, mo, norb, nelec, add_thresh=None, ci0=None, tol=None, lindep=None, max_cycle=None, max_space=None,
           nroots=None, max_memory=None, verbose=None, ecore=0, **kwargs):
    
        # Flags and logging
        log = logger.new_logger(self, verbose)
        if add_thresh is None: add_thresh = self.add_thresh
        ...
        if isinstance(ci0, HCIVec):
            ...
            ci0 = [hf_gnd]
            ci0 = self.enlarge_space(ci0, add_thresh, config_info, excitation_entries, h1e, eri_mo)
            ranks = ci0[0].ranks
            if len(ranks) < nroots:
    ...
    for calculating {nroots} states.\n''')
        ...
        def hop(c):
            ...
            return hc
        precond = lambda x, e, *args: x/(hdiag-e+self.level_shift)

        # Set convergence parameters
        max_len = config_info.combmax_a*config_info.combmax_b
        e_last = 0
        float_tol = self.start_tol
        tol_decay_rate = self.tol_decay_rate
        ...
        for icycle in range(norb):
            ...
            hdiag = self.make_hdiag_slow(ci0[0], config_info, h1e, eri_mo)
        ...
        hdiag = HeatBathCI.make_hdiag_slow(ci0[0], config_info, h1e, eri_mo)
